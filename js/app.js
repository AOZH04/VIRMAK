(() => {
    ...
    document.addEventListener("DOMContentLoaded", (function() {
        const links = document.querySelectorAll(".info_btn a");
        if (links.length > 0) links.forEach((function(link) {
            link.addEventListener("click", (function(e) {
                e.preventDefault();
                const infoBtn = link.closest(".info_btn");
                const parentOfInfoBtn = infoBtn?.parentElement;
                if (parentOfInfoBtn) parentOfInfoBtn.classList.toggle("hidden"); else console.warn("Родитель родителя не найден для ссылки:", link);
            }));
        }));
    }));
    ...
    document.addEventListener("DOMContentLoaded", (() => {
        ...
        if (0 === scrollBlocks.length) return;
        const scrollTriggerCount = 2;
        const scrollStep = 1;
        ...
        document.addEventListener("wheel", (event => {
            for (const block of scrollBlocks) {
                ...
                if (isInside) {
                    ...
                    const atEnd = Math.ceil(scrollLeft) >= Math.floor(maxScrollLeft);
                    const allowVerticalScroll = scrollingRight && atEnd || scrollingLeft && atStart;
                    if (!allowVerticalScroll) {
                        event.preventDefault();
                        if (!scrollStates.has(block)) scrollStates.set(block, {
                            counter: 0,
                            direction: null
                        });
                        const state = scrollStates.get(block);
                        const newDirection = scrollingRight ? "right" : "left";
                        if (state.direction !== newDirection) {
                            state.counter = 0;
                            state.direction = newDirection;
                        }
                        state.counter++;
                        if (state.counter >= scrollTriggerCount) {
                            const step = block.clientWidth * scrollStep;
                            block.scrollLeft += scrollingRight ? step : -step;
                            state.counter = 0;
                        }
                        scrollStates.set(block, state);
                    }
                    break;
                }
            }
    ...
    function updateScrollLine() {
        const scrollblocks = document.querySelectorAll(".scrollblocks .scrollblock");
        const scrollLine = document.querySelector(".horizonscroll");
        if (!scrollLine || 0 === scrollblocks.length) return;
        if (scrollLine.children.length !== scrollblocks.length) {
            scrollLine.innerHTML = "";
            scrollblocks.forEach((() => {
                const item = document.createElement("div");
                item.classList.add("horizonscroll_item");
                scrollLine.appendChild(item);
            }));
        }
        [ ...scrollLine.children ].forEach((item => item.classList.remove("active")));
        const activeIndex = Array.from(scrollblocks).findIndex((el => el.classList.contains("_watcher-view")));
        if (-1 !== activeIndex && scrollLine.children[activeIndex]) scrollLine.children[activeIndex].classList.add("active");
    }
    const scrollblocks = document.querySelectorAll(".scrollblocks .scrollblock");
    scrollblocks.forEach((block => {
        const observer = new MutationObserver((mutations => {
            mutations.forEach((mutation => {
                if ("class" === mutation.attributeName) updateScrollLine();
            }));
        }));
